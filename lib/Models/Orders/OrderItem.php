...
use Indeximstudio\Telegram\Telegram;
...
class OrderItem implements ModelInterface, ArrayAccess
{
    ...
    public function setDeemedResellerCategory($deemed_reseller_category)
    {
        ...
        if (!is_null($deemed_reseller_category) && !in_array($deemed_reseller_category, $allowedValues, true)) {
            (new Telegram("@ERPTeeHuntErrorsBot"))
                ->sendMessages("New undefined `deemed_reseller_category` add it to amazon OrderItem::getDeemedResellerCategoryAllowableValues(). Name: {$deemed_reseller_category}");
